...
class LiveQuadrotorSimulator:
    ...
    def __init__(self, map_file=None):
        ...
        self.dt = 0.01  # 50 Hz for smoother animation
        self.sim_time = 0.0
    ...
    def animated_rrt_planning(self, start=None, goal=None):
        ...
        # self.renderer.getObstaclePoses(np.array(self.env.goal_point))
        ...
        max_iterations = min(3000, min(1000, int(distance * 150)))  # Scale with distance
    ...
    def _create_cube_vertices(self, xmin, ymin, zmin, xmax, ymax, zmax):
        """Create the 8 vertices of an axis-aligned box (no renderer dependency)."""
        return [
    ...
    def initialize_simulation(self, start=None, goal=None):
        """
        Prep everything without opening a live plot (for offline runs).
        Returns True on success.
        """
        # Parse map done in __init__ via map_file; environment already loaded
        if not self.env.set_start_goal_points(start, goal):
            print("Failed to set start/goal points")
            return False

        # Plan path (use the PathPlanner class)
        ...
        # Generate trajectory (same API the live sim expects)
        self.traj_gen = TrajectoryGenerator(self.planner.waypoints)
        ...
        # also seed the trail list so offline plots work
