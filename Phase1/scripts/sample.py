...
imu_path   = "/home/alien/MyDirectoryID_p0/Phase1/Data/Train/IMU/imuRaw3.mat"
vic_path   = "/home/alien/MyDirectoryID_p0/Phase1/Data/Train/Vicon/viconRot3.mat"
...
def gyro_counts_to_rads(counts, n_bias=200):
    ...
    # Adjust scale per your gyro datasheet if needed
...
def estimate_bias(series, ts, window_s=1.5):
    ...
    t0 = ts[0]; mask = ts - t0 <= window_s
    if not np.any(mask): mask = np.arange(min(200, series.size))
    return float(np.mean(series[mask]))

def remap_axes(vecs, M):
    return (M @ vecs.T).T

def estimate_g0(A, ts, window_s=1.5):
    ts = np.asarray(ts).ravel()
    if ts.size == 0: return 1.0
    t0 = ts[0]; mask = (ts - t0) <= window_s
    if not np.any(mask): mask = np.arange(min(200, A.shape[0]))
    mags = np.linalg.norm(A[mask], axis=1)
    g0 = np.median(mags)
    return float(g0 if np.isfinite(g0) and g0 > 1e-6 else 1.0)

# --------- Madgwick with LPF accel + dynamic trust ----------
def madgwick_imu_fused(
    ts, wx, wy, wz, ax, ay, az,
    beta=0.06,                 # fusion aggressiveness (0.02–0.15 typical)
    q0=None,
    gyro_map=np.eye(3),
    accel_map=np.eye(3),
    estimate_gyro_bias=True,
    acc_fc_hz=2.0,             # LPF cutoff for gravity estimate (Hz)
    sigma_g=0.20,              # accel magnitude gate (in g after /g0)
    jerk_thresh_gps=1.0        # jerk gate: |Δa|/Δt in g/s above which we downweight strongly
):
    # shapes
    ts = np.asarray(ts).ravel()
    wx = np.asarray(wx).ravel(); wy = np.asarray(wy).ravel(); wz = np.asarray(wz).ravel()
    ...
    if not all(a.size == N for a in [wx,wy,wz,ax,ay,az]):
        raise ValueError("madgwick_imu_fused: inputs must share the same length")

    # maps
    Gs = np.column_stack([wx, wy, wz])
    As = np.column_stack([ax, ay, az])
    G  = remap_axes(Gs, gyro_map)
    A  = remap_axes(As, accel_map)

    # gyro bias
    if estimate_gyro_bias:
        bx = estimate_bias(G[:,0], ts); by = estimate_bias(G[:,1], ts); bz = estimate_bias(G[:,2], ts)
        G  = G - np.array([bx, by, bz])[None,:]

    # gravity magnitude in sensor units
    g0 = estimate_g0(A, ts)

    # accel LPF (on the vector, one-pole)
    dts = np.diff(ts); dts = np.where(dts<=0, 1e-3, dts)
    Ts  = float(np.median(dts)) if dts.size else 1e-2
    if acc_fc_hz <= 0: alpha = 0.0
    else:
        tau = 1.0/(2*np.pi*acc_fc_hz)
        alpha = float(np.clip(tau/(tau+Ts), 0.0, 0.9999))
    A_lpf = np.zeros_like(A)
    A_lpf[0] = A[0]
    for k in range(1, N):
        A_lpf[k] = alpha*A_lpf[k-1] + (1.0-alpha)*A[k]

    # jerk (change rate of accel magnitude) in g/s — length N with jerk[0]=0
    a_mag = np.linalg.norm(A, axis=1) / g0
    jerk = np.zeros(N)
    if N > 1:
        jerk[1:] = np.abs(np.diff(a_mag)) / np.maximum(dts, 1e-6)
    ...
    for k in range(1, N):
        ...
        # accel correction (use LPF accel as gravity estimate)
        a = A_lpf[k]
        an = np.linalg.norm(a)
        if np.isfinite(an) and an > 1e-12:
            a_n = a / an
            f = np.array([
                2.0*(qx*qz - qw*qy) - a_n[0],
                2.0*(qw*qx + qy*qz) - a_n[1],
                qw*qw - qx*qx - qy*qy + qz*qz - a_n[2]
            ])
            J = np.array([
            ...
            # dynamic trust: magnitude gate and jerk gate (both in g units)
            a_in_g = an / g0
            w_mag  = math.exp(-((a_in_g - 1.0)/sigma_g)**2)
            w_jerk = 1.0 / (1.0 + (jerk[k]/max(1e-6, jerk_thresh_gps))**2)
            w_a = max(0.0, min(1.0, w_mag * w_jerk))

            if np.isfinite(gnorm) and gnorm > 1e-12 and w_a > 1e-4:
                grad /= gnorm
                qDot = qDot_gyro - (beta * w_a) * grad
            else:
                qDot = qDot_gyro
        else:
            qDot = qDot_gyro
    ...
    # to Euler (roll, pitch, yaw)
    Rm = R.from_quat(np.column_stack([Q[:,1],Q[:,2],Q[:,3],Q[:,0]]))  # [x,y,z,w]
    eul = Rm.as_euler('ZYX')    # [yaw, pitch, roll]
    roll  = eul[:,2]; pitch = eul[:,1]; yaw = eul[:,0]
...
def main():
    ...
    # Calibrate accelerometer (consistent everywhere)
    (sx, sy, sz) = scales
    (bax, bay, baz) = biases
    ax = (ax_raw + bax) / sx
    ay = (ay_raw + bay) / sy
    ...
    # ---------------- Madgwick (LPF accel + dynamic trust) ----------------
    GyroMap  = np.eye(3)
    AccelMap = np.eye(3)
    q0_xyzw = R.from_matrix(R0).as_quat()
    q0_wxyz = np.r_[q0_xyzw[3], q0_xyzw[:3]]

    Qmad, mad_angles = madgwick_imu_fused(
        t_imu, wx, wy, wz, ax, ay, az,
        beta=0.06,                 # try 0.04–0.10
        q0=q0_wxyz,
        gyro_map=GyroMap,
        accel_map=AccelMap,
        estimate_gyro_bias=True,
        acc_fc_hz=2.0,             # 1–5 Hz typical
        sigma_g=0.20,              # 0.15–0.30
        jerk_thresh_gps=1.0        # 0.5–2.0 g/s
    )
    ...
    # Unwrap yaw for plotting clarity (optional)
    gyro_yaw_u = np.unwrap(gyro_yaw)
    mad_yaw_u  = np.unwrap(mad_yaw)
    vic_yaw_u  = np.unwrap(vic_yaw)
    ...
    # Yaw (unwrapped for readability)
    axes[2].plot(t_imu, vic_yaw_u,     label='Vicon')
    axes[2].plot(t_imu, gyro_yaw_u,    label='Gyro-only')
    axes[2].plot(t_imu, acc_yaw_lp,    label='Accel-only (LPF)')  # ~zero line
    axes[2].plot(t_imu, mad_yaw_u,     label='Madgwick')
    axes[2].set_xlabel('Time (s)')
    axes[2].set_ylabel('Yaw (rad, unwrapped)')
    ...
    out_path = OUT_DIR / "attitude_all_2D_madgwick_LPF_dynamic.png"
